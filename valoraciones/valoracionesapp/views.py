...
def crear_valoracion(request):
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            ...
            perValorada = requests.get('https://13.38.223.212:8000/api/usuarios/' + str(valoracion['idValorado']) + '/')
            perValorada = perValorada.json()
            reputacion = round(reputacion, 2)
            perValorada['reputacion'] = reputacion
            res = requests.put('https://13.38.223.212:8000/api/usuarios/' + str(perValorada['_id']) + '/', json=perValorada)
